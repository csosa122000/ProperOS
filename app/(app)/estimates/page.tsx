...
export default async function Estimates(){
  ...
  for(const p of (propertyRows||[]) as any[]){
    if(!p.lead_id||(!p.is_primary&&addressByLead.has(p.lead_id)))continue;
    addressByLead.set(p.lead_id,[p.address_line1,p.address_line2,p.city,p.state,p.postal_code].filter(Boolean).join(', '));
  }
  ...
  return <><div className="top"><div><h1>Estimate Builder</h1><p>Build projects directly from the Proper Remodeling master pricing book and keep every estimate tied to its CRM lead.</p></div></div><EstimateBuilder categories={normalizedCategories} items={normalizedItems} leads={leads} organizationId={org.organization_id} userId={user.id}/></>;
}

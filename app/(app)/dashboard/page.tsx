...
export default async function Dashboard(){
  ...
  let perf:Perf|undefined;let leaders:Leader[]=[];let canvassers:Generator[]=[];let telemarketers:Generator[]=[];let posts:CompanyPost[]=[];let monthlyGoal=0;let annualGoal=0;let monthlyLeadCount=0;
  const now=new Date();const periodMonth=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-01`;
  if(org){
    const monthStart=new Date(now.getFullYear(),now.getMonth(),1).toISOString();
    const [{data:performance},{data:leaderboard},{data:canvassing},{data:telemarketing},{data:goals},{count:leadCount},{data:companyPosts}]=await Promise.all([
  ...
  return <>
    ...
    <section className="module-grid section"><div className="card"><h3>Sit Rate</h3><strong>{pct(perf?.sit_rate||0)}</strong><p>{perf?.sit_count||0} sits / {perf?.issued_count||0} issued this month</p></div><div className="card"><h3>Full Demo Rate</h3><strong>{pct(perf?.full_demo_rate||0)}</strong><p>{perf?.full_demo_count||0} full demos</p></div><div className="card"><h3>Close Rate</h3><strong>{pct(perf?.close_rate||0)}</strong><p>{perf?.sale_count||0} sales</p></div></section>

...
export default async function Contracts(){
  ...
  const {data:contractRows}=await supabase.from('contracts').select('id,estimate_id,contract_number,customer_name,contract_price,payment_terms,status,cancellation_notice_acknowledged,created_at').eq('organization_id',org.organization_id).order('created_at',{ascending:false});
  ...
  return <>
    <div className="top"><div><h1>Contracts & Signatures</h1><p>Create contracts only from accepted proposals, manage payment promotions, two homeowners/two representatives, rescission acknowledgement, and signing workflow.</p></div></div>
    <ContractBuilder organizationId={org.organization_id} userId={user.id} proposals={proposals as any}/>
